 ...
 */
const RUNTIME_SERVICE_ACCOUNT = "dibf-223d3@appspot.gserviceaccount.com";
const SENDER = {
 ...
 */
const PUBLIC_SITE_URL = "https://studio-8071626946-149cb.web.app";
/**
...
function getBrevoClient() {
    ...
    if (!apiKeyValue) {
        throw new functions.https.HttpsError("failed-precondition", "BREVO_API_KEY is missing or unavailable to this function.");
    }
    const defaultClient = SibApiV3Sdk.ApiClient.instance;
    const apiKeyAuthentication = defaultClient.authentications["api-key"];
...
function getSafeErrorMessage(error) {
    ...
    if (error && typeof error === "object") {
        const possibleBrevoError = error;
        const parts = [];
        const statusCode = (_a = possibleBrevoError.response) === null || _a === void 0 ? void 0 : _a.statusCode;
        const responseCode = (_c = (_b = possibleBrevoError.response) === null || _b === void 0 ? void 0 : _b.body) === null || _c === void 0 ? void 0 : _c.code;
        const responseMessage = (_e = (_d = possibleBrevoError.response) === null || _d === void 0 ? void 0 : _d.body) === null || _e === void 0 ? void 0 : _e.message;
 ...
 */
function getBrevoMessageId(response) {
    if (!response || typeof response !== "object") {
        return null;
    }
    const result = response;
    const responseBody = result.body;
    if (typeof (responseBody === null || responseBody === void 0 ? void 0 : responseBody.messageId) === "string") {
        return responseBody.messageId;
    }
    const messageIds = responseBody === null || responseBody === void 0 ? void 0 : responseBody.messageIds;
...
function createUnsubscribeUrl(email) {
    return (`${PUBLIC_SITE_URL}/unsubscribe?email=` +
...
exports.sendNewsletter = functions
    ...
    var _a;
    const campaignId = typeof (data === null || data === void 0 ? void 0 : data.campaignId) === "string"
    ...
    const campaignReference = db
        ...
        .doc(campaignId);
    const campaignDocument = await campaignReference.get();
    if (!campaignDocument.exists) {
        throw new functions.https.HttpsError("not-found", "Campaign not found.");
    }
    const campaign = campaignDocument.data();
    const subject = typeof (campaign === null || campaign === void 0 ? void 0 : campaign.subject) === "string"
        ? campaign.subject.trim()
        : "";
    const rawContent = (_a = campaign === null || campaign === void 0 ? void 0 : campaign.content) !== null && _a !== void 0 ? _a : campaign === null || campaign === void 0 ? void 0 : campaign.body;
    const content = typeof rawContent === "string"
        ? rawContent.trim()
        : "";
    if (!subject || !content) {
        throw new functions.https.HttpsError("failed-precondition", "Campaign must have a subject and content.");
    }
    ...
    const subscriberEmails = subscribersSnapshot.docs
        .map((document) => normalizeEmail(document.data().email))
        .filter((email) => email !== null);
    /*
     * Remove duplicate email addresses before sending.
     */
    const subscribers = Array.from(new Set(subscriberEmails));
    if (subscribers.length === 0) {
        throw new functions.https.HttpsError("failed-precondition", "No active subscribers were found.");
    }
    await campaignReference.update({
        status: "sending",
        recipientCount: subscribers.length,
        brevoLastAttemptAt: admin.firestore.FieldValue.serverTimestamp(),
    ...
    try {
        ...
        await campaignReference.update({
            ...
            recipientCount: subscribers.length,
            brevoLastAttemptAt: admin.firestore.FieldValue.serverTimestamp(),
            brevoLastError: admin.firestore.FieldValue.delete(),
            updatedAt: admin.firestore.FieldValue.serverTimestamp(),
        });
        return {
            success: true,
            recipientCount: subscribers.length,
            result: `Successfully sent the newsletter to ` +
                `${subscribers.length} subscriber(s).`,
        };
    }
    catch (error) {
        const safeErrorMessage = getSafeErrorMessage(error);
        console.error("Brevo newsletter delivery failed:", safeErrorMessage);
        try {
            await campaignReference.update({
                status: "failed",
            ...
            console.error("Unable to save the campaign error:", getSafeErrorMessage(updateError));
        }
        throw new functions.https.HttpsError("internal", "An error occurred while sending the newsletter.");
    }
});
/**
 ...
 */
exports.onNewSubscriber = functions
    .runWith({
    secrets: [brevoApiKey],
    serviceAccount: RUNTIME_SERVICE_ACCOUNT,
})
    .firestore.document(`${SUBSCRIBERS_COLLECTION}/{subscriberId}`)
    .onCreate(async (snapshot) => {
    const subscriber = snapshot.data();
    if (!subscriber) {
        console.log(`Subscription ${snapshot.id} has no data.`);
        return null;
    }
    ...
    const status = typeof subscriber.status === "string"
        ...
        : "active";
    if (!email) {
        console.log(`Subscription ${snapshot.id} has an invalid email.`);
        await snapshot.ref.update({
            welcomeEmailSent: false,
            brevoLastError: "Missing or invalid subscriber email address.",
            brevoLastAttemptAt: admin.firestore.FieldValue.serverTimestamp(),
        });
        return null;
    }
    if (status !== "active") {
        console.log(`Subscription ${snapshot.id} is not active. ` +
    ...
    if ((currentSubscriber === null || currentSubscriber === void 0 ? void 0 : currentSubscriber.welcomeEmailSent) === true) {
        console.log(`A welcome email has already been sent to ${email}.`);
        return null;
    }
    const unsubscribeUrl = createUnsubscribeUrl(email);
    const sendSmtpEmail = new SibApiV3Sdk.SendSmtpEmail();
    ...
    try {
        const transactionalEmailsApi = getBrevoClient();
        const response = await transactionalEmailsApi
            .sendTransacEmail(sendSmtpEmail);
        const messageId = getBrevoMessageId(response);
        await snapshot.ref.update(Object.assign({ email, welcomeEmailSent: true, welcomeEmailSentAt: admin.firestore.FieldValue.serverTimestamp(), brevoLastAttemptAt: admin.firestore.FieldValue.serverTimestamp(), brevoLastError: admin.firestore.FieldValue.delete() }, (messageId
            ? {
        ...
        const safeErrorMessage = getSafeErrorMessage(error);
        console.error(`Brevo welcome email failed for ${email}:`, safeErrorMessage);
        try {
            await snapshot.ref.update({
                welcomeEmailSent: false,
                brevoLastError: safeErrorMessage,
            ...
            console.error("Unable to save the welcome email error:", getSafeErrorMessage(updateError));
        }
        throw error;
    }
});
